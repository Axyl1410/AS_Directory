...
const SearchForm = ({ query }: { query?: string }) => {
  return (
    <Form className="relative w-full max-w-[700px]" scroll={false} action="/">
      ...
      />
      <button
        className="absolute right-0 top-1/2 flex h-[25px] w-[25px] -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-black transition-colors hover:bg-black-100 md:h-[40px] md:w-[40px] 2xl:h-[50px] 2xl:w-[50px]"
        type="submit"
      >
        <svg
